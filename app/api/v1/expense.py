...
def get_expense_service() -> ExpenseService:
    return ExpenseService(session = SessionLocal())  # type: ignore


@router.post("/expenses/", response_model=ExpenseCreateModel)
async def create_expense(expense: ExpenseCreateModel, session: AsyncSession = Depends(get_expense_service)):
...
async def read_expense(expense_id: str, session: AsyncSession = Depends(get_expense_service)):
    expense_service = ExpenseService(session)
...
async def read_expenses(skip: int = 0, limit: int = 10, session: AsyncSession = Depends(get_expense_service)):
    expense_service = ExpenseService(session)
    return await expense_service.get_expenses(skip=skip, limit=limit)

@router.delete("/expenses/{expense_id}")
async def delete_expense(expense_id: str, session: AsyncSession = Depends(get_expense_service)):
    expense_service = ExpenseService(session)
    await expense_service.delete_expense(expense_id)
    return {"detail": "Expense deleted"}

@router.get("/expenses/date-range/", response_model=list[ExpenseReadModel])
async def get_expenses_by_date_range(start_date: str, end_date: str, session: AsyncSession = Depends(get_expense_service)):
    expense_service = ExpenseService(session)
...
async def filter_expenses(session: AsyncSession = Depends(get_expense_service), **filters):
    expense_service = ExpenseService(session)
